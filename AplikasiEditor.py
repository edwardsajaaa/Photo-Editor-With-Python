...
class PhotoEditorFinal:
    def __init__(self, root):
        ...
        self.root.title("Foto Editor Final")
    ...
    def create_layout(self):
        ...
        self.status_bar = ttk.Label(self.root, text="Selamat Datang!", anchor=tk.W)
    ...
    # --- Bagian Event Handler & Helper UI ---
    ...
    # --- Bagian State Management & Logika Inti ---
    ...
    def open_image(self):
        ...
        if self.image_path:
            ...
            self.history = [] 
            self.reset_image(save_state=False)
    ...
    # --- Bagian Fungsi Pemrosesan Gambar ---
    ...
    def flip_image(self, direction):
        ...
        if direction == "horizontal":
            self.transformed_image = ImageOps.mirror(self.transformed_image)
        elif direction == "vertical":
            self.transformed_image = ImageOps.flip(self.transformed_image)
        self.apply_all_adjustments()
        self.display_image()
    ...
    def rotate_image(self, angle_degrees):
        if not self.transformed_image: return
        self.save_state()
        self.transformed_image = self.transformed_image.rotate(angle_degrees, expand=True)
        self.apply_all_adjustments()
        self.display_image()
    ...
    def end_crop(self, event):
        if self.crop_rectangle:
            x1, y1, x2, y2 = sorted((self.crop_start_x, self.crop_start_y, event.x, event.y))
            ...
            box_x1 = max(0, x1 - canvas_x_offset)
            box_y1 = max(0, y1 - canvas_y_offset)
            box_x2 = min(new_w, x2 - canvas_x_offset)
            box_y2 = min(new_h, y2 - canvas_y_offset)

            if box_x1 < box_x2 and box_y1 < box_y2:
                real_x1 = int(box_x1 / ratio)
                real_y1 = int(box_y1 / ratio)
                real_x2 = int(box_x2 / ratio)
                real_y2 = int(box_y2 / ratio)
